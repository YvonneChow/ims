$(document).ready(function(){
	...
	d3.tsv("data/localDENovartis_Apps_Compliance-20_Ranges_Percent.txt", function(error, data) {
		...
		rect.on("mouseover",function(d){
				...
				for(var i=0;i<dataSet[index].values.length;i++){
					tooltipSelect.append("<tr><td><span class='color' style='background-color:"+color(dataSet[index].values[i].name)+"'></span><span>"+ dataSet[index].values[i].name +"</span></td><td class='value'>"+(dataSet[index].values[i].number*100).toFixed(1)+"%"+"</td></tr>");
				}
	...
	$.ajax({
        url : "data/localDENovartis_Apps_Compliance-20_Ranges_Percent.txt",
        //dataType: "text",
